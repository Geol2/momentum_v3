...
시간이 지정된 할 일이 다가오면 사용자의 폰으로 푸시 알림을 보냅니다. 앱이 꺼져 있어도
백엔드 스케줄러가 직접 푸시를 쏘기 때문에 도착합니다.
...
백엔드: ReminderScheduler (매분 실행)
   ...
   → public/sw.js 의 push 핸들러가 OS 알림 표시
```
...
알림이 안 오면 백엔드 로그의 `Reminder for todo ... → N endpoint(s)` 를 확인하세요.
